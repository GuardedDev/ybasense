...
--
do -- Folders
    if not isfolder("ybasense") then
        makefolder("ybasense")
    end
    --
    if not isfolder("ybasense/Configs") then
        makefolder("ybasense/Configs")
    end
end
--
do -- Library
    ...
    function Library:UpdateConfigList(List, Type)
        for _, File in listfiles("ybasense/Configs") do
            local FileName = File:gsub("\\", "/"):gsub("ybasense/Configs/", ""):gsub(".cfg", "")
            --
            if Type == "Remove" then
    ...
    function Library:Window(Options)
        ...
        function Library:CreateWatermark()
            ...
            --
            Library:UpdateWatermark(("yba<font color='rgb(%d, %d, %d)'>sense</font>  <font color='rgb(%d, %d, %d)'>%s</font> <font size='10'>FPS</font>  %s"):format(R, G, B, R, G, B, "60", os.date("%X")))
            --
            Library:Notify({
                Message = ("You are using <font color='rgb(%d, %d, %d)'>ybasense</font>. Join <font color='rgb(%d, %d, %d)'>@</font> discord.gg/ybasense"):format(R, G, B, R, G, B),
                Position = "Top Left",
                Delay = 15
            })
            --
                ...
                Library:Connection(RunService.PostSimulation, function()
                    if Library.UI.Initialized and MainWatermark.Visible then
                        ...
                        if (tick() - Watermark.RefreshTick) > Library.UI.WatermarkRefreshRate then
                            Library:UpdateWatermark(("yba<font color='rgb(%d, %d, %d)'>sense</font>  <font color='rgb(%d, %d, %d)'>%s</font> <font size='10'>FPS</font>  %s"):format(R, G, B, R, G, B, FPS, os.date("%X")))
                            --
...
do -- Rage
    ...
    InformationSection:Label({Message = "Updated 15.08.2026", TextColor = Color3.fromRGB(200, 155, 0)})
